...
const HireWorkers = () => {
  ...
  const workerTeams: WorkerTeam[] = [
    ...
    },
    {
      ...
      rating: 4.9,
      completedJobs: 203,
      specialties: ["Industrial Cleaning", "Hazardous Waste", "Bulk Collection"],
      hourlyRate: 35,
      minTeamSize: 3,
      maxTeamSize: 12,
      availability: "Available",
    ...
    },
    {
      ...
      rating: 4.6,
      completedJobs: 98,
      specialties: ["Residential Collection", "Garden Waste", "Emergency Cleanup"],
      hourlyRate: 20,
      minTeamSize: 2,
      maxTeamSize: 6,
      availability: "Busy",
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Connect with professional waste management teams for efficient and reliable service
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-2xl font-semibold text-foreground mb-4">Available Teams</h2>
            
            {workerTeams.map((team) => (
              <Card 
                key={team.id} 
                className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${
                  selectedTeam === team.id ? 'ring-2 ring-primary bg-primary/5' : ''
                }`}
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-semibold text-primary">${team.hourlyRate}/hour per worker</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-muted-foreground">Team Size:</span>
                      <span>{team.minTeamSize}-{team.maxTeamSize} workers</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent className="space-y-6">
                {selectedTeamData && (
                  <div className="p-4 bg-primary/10 rounded-lg">
                    <p className="font-medium text-foreground">{selectedTeamData.name}</p>
                    <p className="text-sm text-muted-foreground">{selectedTeamData.location}</p>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <Select value={workType} onValueChange={setWorkType}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="emergency">Emergency Cleanup (+50%)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <Label htmlFor="teamSize">
                    ...
                    {selectedTeamData && (
                      <span className="text-xs text-muted-foreground ml-2">
                        ({selectedTeamData.minTeamSize}-{selectedTeamData.maxTeamSize} available)
                      </span>
                    )}
                  </Label>
                  <Input
